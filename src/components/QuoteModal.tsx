...
const SuccessMessage = ({ closeModal }: { closeModal: () => void }) => {
  return (
    <div className="py-2">
      <p className="font-twCent text-xl font-bold">
        Ste bližšie k Vášej vysnívanej stavbe z dreva.
      </p>
      ...
      <button
        className="bg-black text-white px-16 py-2 font-twCent uppercase"
        onClick={closeModal}
      >
        Zavrieť
      </button>
...
export const QuoteModal: React.FC<QuoteModalProps> = ({ onClose }) => {
  ...
  async function sendContactEmail(formData: FormData) {
    ...
    try {
      await sendEmail({
        ...
        message: "Vyplneny kontaktny modal",
      });
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50">
      ...
      {/* Modal container */}
      <div className="bg-white p-6 shadow-lg z-10 w-[600px] relative">
        <button
          onClick={closeModal}
            ...
            {!showEmailInput ? (
              ...
              <div>
                <form onSubmit={handleSubmit} className="space-y-4">
                  <h2 className="text-lg font-bold uppercase pt-2">
                    Zanechajte nám Váš email alebo telefón
                  </h2>
                  ...
                  {(error && <ErrorMessage />) || (
                    <button
                      type="submit"
                      className="border border-black text-center text-xs py-2 px-4 flex justify-center items-center text-black uppercase w-full md:w-40 h-10 tracking-wider"
                    >
